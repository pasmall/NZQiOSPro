...
@interface NZQPickPhotoTool () <UINavigationControllerDelegate, UIImagePickerControllerDelegate, TZImagePickerControllerDelegate>
...
@implementation NZQPickPhotoTool
...
- (void)alertAction{
    ...
    [UIAlertController mj_showActionSheetWithTitle:nil message:nil appearanceProcess:^(JXTAlertController * _Nonnull alertMaker) {
        alertMaker.addActionDefaultTitle(@"拍照").addActionDefaultTitle(@"从手机相册选择").addActionCancelTitle(@"取消");
...
@implementation UIViewController (NZQImagePickTool)

- (NSMutableArray<UIImage *> *)nzq_selectedImages {
    NZQPickPhotoTool *pickTool;
    pickTool = objc_getAssociatedObject(self, pickToolKey);
    return pickTool.selectedImages;
}

- (NSMutableArray<PHAsset *> *)nzq_selectedAccests {
    NZQPickPhotoTool *pickTool;
    pickTool = objc_getAssociatedObject(self, pickToolKey);
