...
@interface NZQMyCollectPage ()<NZQVerticalFlowLayoutDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation NZQMyCollectPage
...
- (void)makeData{
    ...
    [[NZQRequestManager sharedManager] GET:BaseUrlWith(DataBuildCollectList) parameters:param completion:^(NZQBaseResponse *response) {
        if (response.error) {
            //错误提示
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        [_collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([NZQMyCollViewCell class]) bundle:[NSBundle mainBundle]] forCellWithReuseIdentifier:NSStringFromClass([NZQMyCollViewCell class])];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dic = self.dataArray[indexPath.row];
    NZQMyCollViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([NZQMyCollViewCell class]) forIndexPath:indexPath];
    cell.dataDic = dic;
    return cell;
}
