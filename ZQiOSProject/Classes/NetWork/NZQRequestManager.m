...
@implementation NZQRequestManager


#pragma mark - POST_GET
// post
- (void)POST:(NSString *)urlString parameters:(id)parameters completion:(void (^)(NZQBaseResponse *))completion
{
    [self request:@"POST" URL:urlString parameters:parameters completion:completion];
}

//get
- (void)GET:(NSString *)urlString parameters:(id)parameters completion:(void (^)(NZQBaseResponse *))completion
{
    [self request:@"GET" URL:urlString parameters:parameters completion:completion];
}

#pragma mark - post & get
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(NZQBaseResponse *response))completion
{
    if (self.isLocal) {
    ...
    if (self.reachabilityManager.networkReachabilityStatus == AFNetworkReachabilityStatusNotReachable) {
        NZQBaseResponse *response = [NZQBaseResponse new];
...
#pragma mark - 加载本地数据
static NSString *jsonFileDirectory = @"NZQLocalJsons"; //项目文件夹路径
- (void)requestLocal:(NSString *)urlString completion:(void (^)(NZQBaseResponse *response))completion
{
...
#pragma mark - 处理数据
- (void)wrapperTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error completion:(void (^)(NZQBaseResponse *response))completion
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        NZQBaseResponse *response = [self convertTask:task responseObject:responseObject error:error];
...
#pragma mark - 包装返回的数据
- (NZQBaseResponse *)convertTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error
{
    
    NZQBaseResponse *response = [NZQBaseResponse new];
    
    if (!NZQIsEmpty(responseObject)) {
...
#pragma mark - 打印返回日志
- (void)LogResponse:(NSString *)urlString response:(NZQBaseResponse *)response
{
    NSLog(@"\n[%@]---%@\n", urlString, response);
}
...
- (void)upload:(NSString *)urlString parameters:(id)parameters formDataBlock:(NSDictionary<NSData *, NZQDataName *> *(^)(id<AFMultipartFormData> formData, NSMutableDictionary<NSData *, NZQDataName *> *needFillDataDict))formDataBlock progress:(void (^)(NSProgress *progress))progress completion:(void (^)(NZQBaseResponse *response))completion
{
    ...
    [self POST:urlString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (datas) {
            [datas enumerateKeysAndObjectsUsingBlock:^(NSData * _Nonnull data, NZQDataName * _Nonnull name, BOOL * _Nonnull stop) {
...
- (void)configSettings
{
    ...
    //自定义处理数据
    self.responseFormat = ^NZQBaseResponse *(NZQBaseResponse *response) {
        return response;
    };
}
...
+ (instancetype)manager {
    NZQRequestManager *manager = [super manager];
    [manager configSettings];
    return manager;
}

static NZQRequestManager *_instance = nil;
